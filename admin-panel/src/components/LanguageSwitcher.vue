<template>
  <div class="relative" ref="languageDropdown">
    ...
    >
      ...
      <ChevronDownIcon class="w-2 h-2 sm:w-3 sm:h-3 text-white/60 transition-transform duration-200" :class="{ 'rotate-180': showLanguageMenu }" />
    </button>
    ...
    >
      <div
        ...
        class="absolute right-0 mt-2 w-36 sm:w-40 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-50"
      >
        <button
          v-for="lang in supportedLanguages"
          :key="lang"
          @click="changeLanguage(lang)"
          class="w-full text-left px-3 sm:px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 flex items-center space-x-2"
          :class="{ 'bg-gray-100 font-medium': locale === lang }"
        >
