use shortlinker::errors::{ShortlinkerError, Result};
use std::error::Error;
...
mod error_conversion_tests {
    ...
    fn test_io_error_conversion() {
        ...
        assert!(matches!(shortlinker_error, ShortlinkerError::FileOperation(_)));
    ...
    fn test_serde_json_error_conversion() {
        ...
        assert!(matches!(shortlinker_error, ShortlinkerError::Serialization(_)));
...
mod error_message_tests {
    ...
    fn test_error_message_format() {
        let test_cases = vec![
            (ShortlinkerError::database_connection("连接超时"), "数据库连接错误: 连接超时"),
            (ShortlinkerError::database_operation("查询失败"), "数据库操作错误: 查询失败"),
            (ShortlinkerError::file_operation("权限不足"), "文件操作错误: 权限不足"),
            (ShortlinkerError::validation("格式错误"), "验证错误: 格式错误"),
            (ShortlinkerError::not_found("用户不存在"), "资源未找到: 用户不存在"),
            (ShortlinkerError::serialization("JSON错误"), "序列化错误: JSON错误"),
            (ShortlinkerError::signal_operation("信号中断"), "信号操作错误: 信号中断"),
        ];
...
mod error_chain_tests {
    ...
    fn test_error_propagation() {
        ...
        fn higher_level_operation() -> Result<String> {
            operation_that_fails().map_err(|e| {
                ShortlinkerError::validation(format!("高层错误: {}", e))
            })
        }
    ...
    fn test_multiple_error_conversions() {
        ...
        // 再次包装错误
        let wrapped_error = ShortlinkerError::validation(format!("包装错误: {}", shortlinker_error));
...
mod integration_tests {
    ...
    fn test_error_in_real_scenarios() {
        ...
        fn simulate_resource_lookup(id: &str) -> Result<String> {
            if id == "nonexistent" {
                return Err(ShortlinkerError::not_found(format!("ID {} 对应的资源不存在", id)));
            }
            Ok("found".to_string())
        }
